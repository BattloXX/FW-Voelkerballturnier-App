...
def _points_draw(tournament: models.Tournament) -> int:
    return getattr(tournament, "points_draw", 1)


def _apply_match(stats, match, tournament):
    ...
    if match.score_a > match.score_b:
        ...
        draw_pts = _points_draw(tournament)
        stats[match.team_a_id]["points"] += draw_pts
        stats[match.team_b_id]["points"] += draw_pts
...
def calculate_standings(tournament: models.Tournament, field_group: int, round_type: models.RoundType, db: Session) -> List[StandingEntry]:
    ...
    for match in matches:
        ...
        if match.score_a is None or match.score_b is None:
            continue
        _apply_match(stats, match, tournament)
    ...
    for i, s in enumerate(sorted_teams):
        result.append(StandingEntry(
            ...
            organization=getattr(s["team"], "organization", None),
            played=s["played"],
            wins=s["wins"],
            losses=s["losses"],
            points=s["points"],
...
def calculate_group_standings(tournament: models.Tournament, group: int, round_type: models.RoundType, db: Session) -> List[StandingEntry]:
    matches = db.query(models.Match).filter(
        models.Match.tournament_id == tournament.id,
        models.Match.round_type == round_type,
        models.Match.field_number == group,
        models.Match.status == models.MatchStatus.finished
    ).all()
...
def calculate_inter_standings(tournament: models.Tournament, inter_group: int, db: Session) -> List[StandingEntry]:
    return calculate_group_standings(tournament, inter_group, models.RoundType.inter, db)
